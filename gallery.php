					...
					<div class="centered-inner photo-feed js-masonry flexed" id="photo-feed">
						...
						<div class="image-block" id="post-<?php the_ID(); ?>">
							...
							<div class="image-text boxsized">
								<?php
									printf(__('<time class="updated date-stamp" datetime="%1$s" pubdate><span class="fa calendar-icon"></span>%2$s %3$s</time>', 'bonestheme'), get_the_time('Y-m-j'), human_time_diff( get_the_time('U'), current_time('timestamp') ), __('ago', 'patrikblom'));
								?>
							...
							<div class="image-block-footer clearfix">
								...
								<?php
									$terms_list = '';
									$num_terms = count(wp_get_post_terms( $post->ID, $taxonomy ));
								?>
								<?php if($num_terms < 3) : ?>
